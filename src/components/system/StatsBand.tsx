...
export function StatsBand({ locale }: { locale: Locale }) {
  ...
  const fill = (v: string) => v.replace("{n}", String(technologies.length)).replace("{s}", String(company.showrooms.length));
  return (
      ...
      <RevealGroup className="mx-auto grid max-w-7xl grid-cols-2 px-3 md:px-7 lg:grid-cols-4">
        {stats.map((s, i) => (
          ...
          >
            ...
            <p className="display mt-3 flex items-baseline gap-2 text-[clamp(3.2rem,8.5vw,8.5rem)] leading-[0.85] text-deep">
